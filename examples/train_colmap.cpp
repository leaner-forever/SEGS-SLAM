    ...
    const std::filesystem::path& path)
{
    ...
    std::cout << std::endl;
    pMapper->num_cameras_=num_cameras;
    pMapper->gaussians_->setApperance();
}
    ...
    const std::filesystem::path& images_dir)
{
    ...
    for (std::size_t i = 0; i < num_reg_images; ++i) {
        ...
        std::cout.flush();
        // if(image_name == "30181_0_gt.jpg") {
        // std::cout << image_name << "xyz:  " << tx << " " << ty
        //     << " " << tz << qw << " " << qx << " " << qy<< " " << qz<< "------ " << std::endl;
        // }
...
//PSNR_GS 31.22136688  SSIM:: 0.94345945