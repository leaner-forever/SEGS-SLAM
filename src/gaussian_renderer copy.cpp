    ...
    bool use_override_color)
{
    ...
    auto mask_fine = mask; 
    auto neural_opacity_fine = neural_opacity; 
    auto scaling_fine = scaling_;             
    auto screenspace_points_fine = torch::zeros_like(xyz_,
        torch::TensorOptions().dtype(pc->get_anchor().dtype()).requires_grad(true).device(torch::kCUDA));

    if (pc->use_coarse_anchor)
    {
        bool is_training_coarse = pc->mlp_color_c->is_training();
        auto neural_gaussians_coarse = generate_neural_gaussians_coarse(
            viewpoint_camera,
            image_height,
            image_width,
            pc,
            visible_mask,
            is_training_coarse);
        auto xyz_c = std::get<0>(neural_gaussians_coarse);
        auto color_c = std::get<1>(neural_gaussians_coarse);
        auto opacity_c = std::get<2>(neural_gaussians_coarse);
        auto scaling_c = std::get<3>(neural_gaussians_coarse);
        auto rot_c = std::get<4>(neural_gaussians_coarse);

        torch::Tensor neural_opacity_c, mask_c;
        if (is_training_coarse) {
            neural_opacity_c = std::get<5>(neural_gaussians_coarse);
            mask_c = std::get<6>(neural_gaussians_coarse);
        }

        xyz_ = torch::cat({xyz_, xyz_c}, 0); 
        color_ = torch::cat({color_, color_c}, 0);
        opacity_ = torch::cat({opacity_, opacity_c}, 0);
        scaling_ = torch::cat({scaling_, scaling_c}, 0);
        rot_ = torch::cat({rot_, rot_c}, 0);
        if (is_training_coarse) {
            neural_opacity = torch::cat({neural_opacity, neural_opacity_c}, 0);
            mask = torch::cat({mask, mask_c}, 0);       
        }
    }
    ...
    if(retain_grad)
    {
        try {
            ...
            screenspace_points_fine.retain_grad();
        }
    ...
    if (pc->use_coarse_anchor)
    {
        auto radii_fine = radii.slice(0, 0, screenspace_points_fine.size(0));
...
std::tuple<torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor, torch::Tensor>
GaussianRenderer::generate_neural_gaussians_coarse(
    std::shared_ptr<GaussianKeyframe> viewpoint_camera,
    int image_height,
    int image_width,
    std::shared_ptr<GaussianModel> pc,
    ...
    bool is_training)
{
    auto feat = pc->_anchor_feat_c;
    auto anchor = pc->_anchor_c;
    auto grid_offsets = pc->_offset_c;
    auto grid_scaling = pc->_scaling_c;
    auto ob_view = anchor - viewpoint_camera->camera_center_;
    auto ob_dist = torch::frobenius_norm(ob_view, /*dim=*/{1}, /*keepdim=*/true);
    ob_view = ob_view / ob_dist;
    ...
    if (pc->use_feat_bank){
        ...
        auto bank_weight = pc->mlp_feature_bank_c->forward(cat_view).unsqueeze(/*dim=*/1); 
    ...
    if (pc->appearance_dim_coarse > 0)
    {
        ...
        appearance_feat = pc->mlp_apperance_c->forward(ob_pose_expanded);
    }

    torch::Tensor neural_opacity;
    if (pc->add_opacity_dist)
        neural_opacity = pc->mlp_opacity_c->forward(cat_local_view);
    else
        neural_opacity = pc->mlp_opacity_c->forward(cat_local_view_wodist);
    ...
    {
        if(pc->add_color_dist )
            color = pc->mlp_color_c->forward(torch::cat({cat_local_view, appearance_feat}, 1));
        else
            color = pc->mlp_color_c->forward(torch::cat({cat_local_view_wodist, appearance_feat}, 1));
    }
    else
    {
        if(pc->add_color_dist )
            color = pc->mlp_color_c->forward(cat_local_view);
        else
            color = pc->mlp_color_c->forward(cat_local_view_wodist);
    }
    color = color.reshape({anchor.size(0) * pc->n_offsets_coarse, 3});
    torch::Tensor scale_rot;
    if(pc->add_cov_dist)
        scale_rot = pc->mlp_cov_c->forward(cat_local_view);
    else
        scale_rot = pc->mlp_cov_c->forward(cat_local_view_wodist);
    scale_rot = scale_rot.reshape({anchor.size(0) * pc->n_offsets_coarse, 7}); 
    auto offsets = grid_offsets.view({-1, 3});
    ...
    torch::Tensor concatenated_repeated;
    {
        ...
        int n = sizes[0];
        int c = sizes[1];
        concatenated_repeated = concatenated.repeat({1, pc->n_offsets_coarse});
        concatenated_repeated = concatenated_repeated.view({n * pc->n_offsets_coarse, c});
    }
