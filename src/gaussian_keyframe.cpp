...
void GaussianKeyframe::setCameraParams(const Camera& camera, cv::MatSize imgsize)
{
    this->camera_id_ = camera.camera_id_;
    this->camera_model_id_ = camera.model_id_;
    this->image_height_ = imgsize[0];
    this->image_width_ = imgsize[1];
...
void GaussianKeyframe::computeTransformTensors4Eval()
{
    if (this->set_pose_ && this->set_camera_) {
        this->world_view_transform_ = tensor_utils::EigenMatrix2TorchTensor(
            this->getWorld2View2(this->trans_, this->scale_),
        ...
        this->camera_center_ = torch::empty({3}, torch::kCUDA);
        this->camera_center_ = this->world_view_transform_.inverse().index({3, torch::indexing::Slice(0, 3)});
        std::cout << "camera_center_: " << this->camera_center_;
    ...
    std::cout << "computeTransformTensors4Eval end" << this->camera_center_;
}
...
void GaussianKeyframe::logger()
{
    std::cout << "[GaussianKeyframe]fid_: " << fid_ << ", camera_id_ = " << camera_id_
              << ", FoVx_ = " << FoVx_ << ", FoVy_ = " << FoVy_
              << ", image_width_ = " << image_width_ << ", image_height_ = " << image_height_
              << ", world_view_transform_ = " << world_view_transform_
              << ", projection_matrix_ = " << projection_matrix_
              << ", full_proj_transform_ = " << full_proj_transform_
              << ", camera_center_ = " << camera_center_
              << std::endl;
}