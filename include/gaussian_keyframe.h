...
public:
    ...
    void setCameraParams(const Camera& camera, cv::MatSize imgsize);
    ...
    void computeTransformTensors4Eval();
...
public:
    ...
    int image_width_;              
    int image_height_;             
    torch::Tensor edge_image_; 
    torch::Tensor edge_mask_; 
    ...
    std::vector<torch::Tensor> gaus_pyramid_original_image_; 
    ...
    Eigen::Quaterniond R_quaternion_;  
    Eigen::Vector3d t_;                
    Sophus::SE3d Tcw_;                 

    torch::Tensor R_tensor_; 
    torch::Tensor t_tensor_; 
